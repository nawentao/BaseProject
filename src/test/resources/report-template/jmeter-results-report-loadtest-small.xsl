...
<xsl:template name="summary">
    ...
    <table align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
        ...
        <tr valign="top">
            <xsl:variable name="allCount" select="count(/testResults/*[not(contains(@lb,'初始化'))])" />
            <xsl:variable name="allFailureCount" select="count(/testResults/*[attribute::s='false'and not(contains(@lb,'初始化'))])" />
            <xsl:variable name="allSuccessCount" select="count(/testResults/*[attribute::s='true' and not(contains(@lb,'初始化'))])" />
            <xsl:variable name="allSuccessPercent" select="$allSuccessCount div $allCount" />
            <xsl:variable name="allTotalTime" select="sum(/testResults/*[ not(contains(@lb,'初始化'))]/@t)" />
            <xsl:variable name="allAverageTime" select="$allTotalTime div $allCount" />
            <xsl:variable name="allMinTime">
                <xsl:call-template name="min">
                    <xsl:with-param name="nodes" select="/testResults/*[ not(contains(@lb,'初始化'))]/@t" />
                </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:variable name="allEndTime">
                <xsl:for-each select="/testResults/*[not(contains(@lb,'初始化'))]/@ts">
...
<xsl:template name="pagelist">
    ...
    <table align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<!--
			...
			sso_cas_index
			【初始化】识别当前操作系统是windows还是linux
			<xsl:for-each select="/testResults/*[@lb!='【初始化】识别当前操作系统是windows还是linux'and not(@lb = preceding::*/@lb)]">
		-->
        <xsl:for-each select="/testResults/*[not(contains(@lb,'初始化'))and not(@lb = preceding::*/@lb)]">
